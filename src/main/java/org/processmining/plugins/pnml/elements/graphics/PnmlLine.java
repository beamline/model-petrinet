...
public class PnmlLine extends PnmlElement {
	...
	public void convertToNet(AbstractGraphElement element) {
		if (shape != Shape.SHAPE_DEFAULT) {
			...
			element.getAttributeMap().put(AttributeMap.STYLE, 11 /*GraphConstants.STYLE_SPLINE*/);
		}
		if (color != null) {
			/*
			 * Set color.
	...
	public PnmlLine convertFromNet(AbstractGraphElement element) {
		PnmlLine line = null;
		try {
			int style = element.getAttributeMap().get(AttributeMap.STYLE, 11/*GraphConstants.STYLE_SPLINE*/);
//			switch (style) {
